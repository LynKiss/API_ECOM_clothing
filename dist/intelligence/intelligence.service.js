...
let IntelligenceService = class IntelligenceService {
    ...
    async getProductRecommendations(query) {
        ...
        if (query.productId) {
            ...
            if (model.status === 'trained' &&
                ...
                const recommendations = this.recommendByMatrixFactorization(model, query.productId, products, limit);
            ...
            const baseOrderCount = await this.countOrdersContainingProduct(query.productId, since);
            const recommendations = await this.getCoPurchaseRecommendations(query.productId, since, limit, baseOrderCount);
            if (recommendations.length > 0) {
                return {
                    mode: 'co_purchase',
                    model: {
                        ...
                        status: 'fallback',
                        reason: model.status === 'insufficient_data'
                            ? model.reason
        ...
        return {
            ...
            model: {
                ...
                status: 'fallback',
                reason: 'No source product was provided, so personalized/item-similarity recommendations cannot be trained for a target item.',
            },
            sourceProduct: null,
            historyDays,
            items: await this.getPopularProductRecommendations(since, limit, query.productId),
        };
    }
    async getDemandForecast(query) {
        ...
        const history = await this.getDailyDemandSeries(product.productId, historyDays);
        const stats = this.calculateDemandStats(history);
        const model = this.trainDemandRegressionModel(history);
        const forecast = model.status === 'trained'
            ? this.buildRegressionForecast(history, horizonDays, model)
            : this.buildForecast(history, horizonDays);
        const totalForecastDemand = forecast.reduce((sum, point) => sum + point.forecastQuantity, 0);
        return {
            ...
            summary: {
                ...
                projectedStockAfterHorizon: Number((product.quantityAvailable - totalForecastDemand).toFixed(2)),
                confidence: this.getForecastConfidence(history),
            },
        };
    }
    async getReorderSuggestions(query) {
        const historyDays = query.historyDays ?? 90;
        ...
        const suggestions = products
            .map((product) => this.buildReorderSuggestion(product, demandByProduct.get(product.productId) ??
            this.emptyDemandSeries(historyDays), leadTimeDays, coverageDays))
    ...
    async countOrdersContainingProduct(productId, since) {
    ...
    async getCoPurchaseRecommendations(productId, since, limit, baseOrderCount) {
        const rows = await this.orderItemsRepository
            .createQueryBuilder('target')
            .innerJoin(order_item_entity_1.OrderItemEntity, 'candidate', 'candidate.order_id = target.order_id AND candidate.product_id <> :productId', { productId })
            .innerJoin(order_entity_1.OrderEntity, 'order', 'order.order_id = target.order_id')
            .innerJoin(product_entity_1.ProductEntity, 'product', 'product.product_id = candidate.product_id')
            .leftJoin(product_image_entity_1.ProductImageEntity, 'image', 'image.product_id = product.product_id AND image.is_primary = 1')
    ...
    async getPopularProductRecommendations(since, limit, excludeProductId) {
    ...
    async getRecommendationInteractions(since) {
        const rows = await this.orderItemsRepository
            .createQueryBuilder('item')
            .innerJoin(order_entity_1.OrderEntity, 'order', 'order.order_id = item.order_id')
    ...
    trainMatrixFactorization(interactions) {
        ...
        if (users.length < 2 || products.length < 2 || interactions.length < 4) {
            return {
                ...
                reason: 'Can toi thieu 2 khach hang, 2 san pham va 4 interaction mua hang de train matrix factorization.',
            };
        }
        const userIndex = new Map(users.map((userId, index) => [userId, index]));
        const productIndex = new Map(products.map((productId, index) => [productId, index]));
        const factors = Math.min(12, Math.max(4, Math.floor(Math.sqrt(products.length))));
        const epochs = 80;
        const learningRate = 0.035;
        const regularization = 0.025;
        const seed = 1337;
        const random = this.seededRandom(seed);
        const userFactors = users.map(() => Array.from({ length: factors }, () => (random() - 0.5) * 0.1));
        const itemFactors = products.map(() => Array.from({ length: factors }, () => (random() - 0.5) * 0.1));
        ...
        return {
            ...
            itemBias,
            globalMean,
            evaluation: this.evaluateMatrixFactorization(interactions, users, products, itemFactors),
        };
    }
    recommendByMatrixFactorization(model, sourceProductId, products, limit) {
        ...
        return model.products
            .map((productId, index) => ({
            productId,
            similarity: this.cosineSimilarity(sourceVector, model.itemFactors[index]),
        }))
            ...
            .filter((item) => Boolean(item.product?.isShow))
            ...
            reason: 'Duoc goi y boi mo hinh matrix factorization dua tren hanh vi mua hang cua khach hang.',
        }));
    }
    evaluateMatrixFactorization(interactions, users, products, itemFactors) {
        ...
        for (const userId of users) {
            ...
            if (sourceIndex < 0 || holdoutIndex < 0)
                continue;
    ...
    async getDailyDemandSeries(productId, historyDays) {
        ...
        const raw = await this.orderItemsRepository
            ...
            .orderBy('date', 'ASC')
            .getRawMany();
        const quantityByDate = new Map(raw.map((row) => [this.toDateKey(row.date), Number(row.quantity)]));
        return this.dateRange(start, end).map((date) => ({
            date: this.toDateKey(date),
            quantity: quantityByDate.get(this.toDateKey(date)) ?? 0,
        }));
    }
    async getAllProductDailyDemand(since) {
    ...
    groupDemandByProduct(rows, historyDays) {
        const end = this.startOfDay(new Date());
        const start = this.daysAgo(historyDays - 1, end);
        const baseDates = this.dateRange(start, end).map((date) => this.toDateKey(date));
        const grouped = new Map();
        for (const row of rows) {
            const dateKey = this.toDateKey(row.date);
            const current = grouped.get(row.productId) ?? new Map();
        ...
        for (const [productId, quantityByDate] of grouped.entries()) {
            result.set(productId, baseDates.map((date) => ({
                date,
    ...
    buildReorderSuggestion(product, demand, leadTimeDays, coverageDays) {
        const stats = this.calculateDemandStats(demand);
        const forecastModel = this.trainDemandRegressionModel(demand);
        const planningForecast = forecastModel.status === 'trained'
            ? this.buildRegressionForecast(demand, leadTimeDays + coverageDays, forecastModel)
        ...
        const coverageDemand = planningForecast.reduce((sum, point) => sum + point.forecastQuantity, 0);
        const avgDailyDemand = planningForecast.length > 0
            ? coverageDemand / planningForecast.length
            : stats.weightedAverageDailyDemand;
        const safetyStock = Math.ceil(stats.stdDev30 * Math.sqrt(leadTimeDays) * 1.65);
        const reorderPoint = Math.ceil(leadTimeDemand + safetyStock);
        const targetStock = Math.ceil(coverageDemand + safetyStock);
        const suggestedOrderQty = Math.max(0, targetStock - product.quantityAvailable);
        const daysUntilStockout = avgDailyDemand > 0
            ? Number((product.quantityAvailable / avgDailyDemand).toFixed(1))
            : null;
        const shouldReorder = avgDailyDemand > 0 && product.quantityAvailable <= reorderPoint;
        const urgency = this.getReorderUrgency(shouldReorder, product.quantityAvailable, reorderPoint, daysUntilStockout, leadTimeDays);
        return {
            product: this.mapProduct(product),
            ...
            shouldReorder,
            urgency,
            model: forecastModel.status === 'trained'
                ? {
            ...
            reason: this.buildReorderReason(product.quantityAvailable, reorderPoint, daysUntilStockout, leadTimeDays),
        };
    }
    trainDemandRegressionModel(history) {
        ...
        if (rows.length < 21 ||
            ...
            return {
                ...
                status: 'insufficient_data',
                reason: 'Can toi thieu 21 training rows va 4 ngay co ban hang de train time-series regression.',
            };
        }
        ...
        const weights = this.fitRidgeRegression(normalized.train.map((row) => row.features), normalized.train.map((row) => row.target), 0.08, 0.01, 900);
        const predictions = normalized.test.map((row) => Math.max(0, this.dot(weights, row.features)));
    ...
    buildRegressionForecast(history, horizonDays, model) {
        ...
        return Array.from({ length: horizonDays }, (_, index) => {
            ...
            const features = rawFeatures.map((value, featureIndex) => featureIndex === 0
                ? 1
    ...
    buildTimeSeriesTrainingRows(history) {
        const rows = [];
        for (let index = 14; index < history.length; index += 1) {
            rows.push({
                features: this.buildTimeSeriesFeatures(history.slice(0, index), new Date(history[index].date)),
                target: history[index].quantity,
            });
        }
        return rows;
    }
    buildTimeSeriesFeatures(history, date) {
    ...
    normalizeTrainingRows(trainRows, testRows) {
        const featureCount = trainRows[0]?.features.length ?? 0;
        const means = Array.from({ length: featureCount }, (_, featureIndex) => featureIndex === 0
            ? 0
        ...
        const scales = Array.from({ length: featureCount }, (_, featureIndex) => {
            ...
            const variance = this.average(trainRows.map((row) => Math.pow(row.features[featureIndex] - mean, 2)));
            const scale = Math.sqrt(variance);
            return scale > 0 ? scale : 1;
        });
        const normalize = (row) => ({
            target: row.target,
            features: row.features.map((value, featureIndex) => featureIndex === 0
                ? 1
    ...
    fitRidgeRegression(features, targets, learningRate, regularization, epochs) {
    ...
    buildForecast(history, horizonDays) {
        const stats = this.calculateDemandStats(history);
        ...
        return Array.from({ length: horizonDays }, (_, index) => {
            ...
            const forecastQuantity = Math.max(0, stats.weightedAverageDailyDemand * factor);
            return {
                date: this.toDateKey(date),
                forecastQuantity: Number(forecastQuantity.toFixed(2)),
    ...
    calculateDemandStats(history) {
        ...
        const weightedAverageDailyDemand = last30.length >= 30 ? avg7 * 0.6 + avg30 * 0.4 : avgAll;
        return {
    ...
    calculateWeeklySeasonality(history) {
    ...
    getForecastConfidence(history) {
        const activeDays = history.filter((point) => point.quantity > 0).length;
        if (history.length >= 90 && activeDays >= 15)
            return 'high';
        if (history.length >= 30 && activeDays >= 5)
            return 'medium';
        return 'low';
    }
    getReorderUrgency(shouldReorder, currentStock, reorderPoint, daysUntilStockout, leadTimeDays) {
        if (!shouldReorder)
            return 'none';
    ...
    buildReorderReason(currentStock, reorderPoint, daysUntilStockout, leadTimeDays) {
    ...
    cosineSimilarity(left, right) {
        const denominator = Math.sqrt(this.dot(left, left)) * Math.sqrt(this.dot(right, right));
        if (denominator === 0)
            return 0;
    ...
    mae(actual, predicted) {
        if (actual.length === 0)
            return 0;
        return this.average(actual.map((value, index) => Math.abs(value - (predicted[index] ?? 0))));
    }
    rmse(actual, predicted) {
        if (actual.length === 0)
            return 0;
        return Math.sqrt(this.average(actual.map((value, index) => Math.pow(value - (predicted[index] ?? 0), 2))));
    }
    mape(actual, predicted) {
        ...
        if (rows.length === 0)
            return null;
        const value = this.average(rows.map((row) => Math.abs(row.actual - row.predicted) / row.actual)) * 100;
